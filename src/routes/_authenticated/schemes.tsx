...
export const Route = createFileRoute("/_authenticated/schemes")({
  head: () => ({ meta: [{ title: "Government Schemes — Krishi Bondhu" }] }),
  loader: ({ context }) => context.queryClient.ensureQueryData(schemesQuery),
  component: () => (
    <AppShell><Suspense fallback={null}><SchemesPage /></Suspense></AppShell>
  ),
});
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground">{t("govSchemesDesc")}</p>
      </div>
          ...
          <article key={s.id} className="rounded-2xl border border-border bg-[image:var(--gradient-card)] p-4 shadow-[var(--shadow-soft)]">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <span className="inline-block rounded-full bg-[color:var(--saffron)]/20 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-[color:var(--saffron-foreground)]">{s.category}</span>
                <h3 className={`mt-1 text-base font-bold leading-snug text-foreground ${lang === "bn" ? "font-bangla" : ""}`}>{lang === "bn" ? s.title_bn : s.title_en}</h3>
                {s.ministry && <p className="mt-0.5 text-[11px] text-muted-foreground">{s.ministry}</p>}
              </div>
            </div>
            ...
            {(lang === "bn" ? s.eligibility_bn : s.eligibility_en) && (
              <div className="mt-3 rounded-xl bg-muted px-3 py-2">
                <div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{t("eligibility")}</div>
